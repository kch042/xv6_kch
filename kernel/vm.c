...
// map user's pagetable to kpagetable
void
kvmmapuser(pagetable_t upt, pagetable_t kpt, uint64 oldsz, uint64 newsz) {
	pte_t *upte, *kpte;
	uint64 va, pa;
	int flags;

	// note that mapping of va should be page-aligned
	for (va = PGROUNDUP(oldsz); va < newsz; va += PGSIZE) {
		upte = walk(upt, va, 0);
		if (upte == 0)
			panic("kvmmapuser: upte not found");
		kpte = walk(kpt, va, 1);
		if (kpte == 0)
			panic("kvmmapuser: kpte not found");
		
		// clear PTE_U bit to access that page
		// in kernel mode
